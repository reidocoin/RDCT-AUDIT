 ...
 * @version 0.1.5
 */
    private $data = array('name' => '', 'senha' => '', 'email' => '', 'ativo' => 0, 'lastlogin' => 0, 'acesso' => '1', 'endereco' => '', 'fone_casa' => '', 'fone_cel' => '', 'nascimento' => '', 'sexo' => '', 'escolaridade' => '', 'email_old' => '', 'Admissao' => '', 'ramal' => '', 'cargo' => '', 'cad_data' => '', 'equipe' => '');
 ...
 */
    public function load($id)
    {
       ...
       $this->data = $this->db->query('SELECT ' . $this->db->fieldName('id') . ', ' . $this->db->fieldName('name') . ', ' . $this->db->fieldName('senha') . ', ' . $this->db->fieldName('email') . ', ' . $this->db->fieldName('ativo') . ', ' . $this->db->fieldName('lastlogin') . ', '. $this->db->fieldName('acesso') . ', ' . $this->db->fieldName('ramal') . ', ' . $this->db->fieldName('cargo') . ' FROM ' . $this->db->tableName('usuarios') . ' WHERE ' . $this->db->fieldName('id') . ' = ' . (int) $id)->fetch();
    }
