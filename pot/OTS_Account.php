 ...
 * @version 0.1.5
 */
    private $data = array('name' => '', 'senha' => '', 'email' => '', 'ativo' => 0, 'lastlogin' => 0, 'acesso' => '1', 'endereco' => '', 'fone_casa' => '', 'fone_cel' => '', 'nascimento' => '', 'sexo' => '', 'escolaridade' => '', 'email_old' => '', 'equipe' => '', 'avatar' => '', 'filial' => '');
 ...
 */
    ...
    public function createNamed($name = null)
    {
        ...
        if( !isset($name) )
        {
            ...
            foreach( $this->db->query('SELECT ' . $this->db->fieldName('name') . ' FROM ' . $this->db->tableName('usuarios') )->fetchAll() as $account)
            {
                $exist[] = $account['name'];
            }
        ...
        $this->db->query('INSERT INTO ' . $this->db->tableName('usuarios') . ' (' . $this->db->fieldName('name') . ', ' . $this->db->fieldName('password') . ', ' . $this->db->fieldName('email') . ', ) VALUES (' . $this->db->quote($name) . ', \'\', \'\')');
 ...
 */
    public function load($id)
    {
       ...
       $this->data = $this->db->query('SELECT ' . $this->db->fieldName('id') . ', ' . $this->db->fieldName('name') . ', ' . $this->db->fieldName('senha') . ', ' . $this->db->fieldName('email') . ', ' . $this->db->fieldName('ativo') . ', ' . $this->db->fieldName('lastlogin') . ', '. $this->db->fieldName('acesso') . ', '. $this->db->fieldName('equipe') . ', '. $this->db->fieldName('avatar') . ' FROM ' . $this->db->tableName('usuarios') . ' WHERE ' . $this->db->fieldName('id') . ' = ' . (int) $id)->fetch();
    }
 ...
 */
    ...
    {
        // finds player's ID
        $id = $this->db->query('SELECT ' . $this->db->fieldName('id') . ' FROM ' . $this->db->tableName('usuarios') . ' WHERE ' . $this->db->fieldName('email') . ' = ' . $this->db->quote($name) )->fetch();
 ...
 */
    ...
    {
        if( !isset($this->data['equipe']) ){
            throw new E_OTS_NotLoaded();
        }else{
			if($this->data['equipe'] == '1'){
				return 'Negociador';
			}elseif($this->data['equipe'] == '2'){
				return 'Administrativo';	
			}elseif($this->data['equipe'] == '3'){
				return 'Supervisao';
			}elseif($this->data['equipe'] == '4'){
				return '--N identificado --';
			}elseif($this->data['equipe'] == '5'){
				return 'Sindicato';
			}elseif($this->data['equipe'] == '6'){
				return 'Pesquisa';
			}else{
				return 'Não definido';
			}
	 ...
	 public function getAvatarUser()
    {
        if( !isset($this->data['avatar']) ){
			...
			if($this->data['avatar'] == "0"){
				return	'user/imagem/profile.jpg';
			}else{
    ...
    {
        if( !isset($this->data['filial']) ){
            throw new E_OTS_NotLoaded();
        }else{
			if($this->data['filial'] == '1'){
				return 'Rio Grande do Sul';
			}elseif($this->data['filial'] == '2'){
				return 'Matriz';	
			}elseif($this->data['filial'] == '3'){
				return 'Não Definido 3';
			}elseif($this->data['filial'] == '4'){
				return 'Rio de Janeiro';
			}elseif($this->data['filial'] == '5'){
				return 'Parana';
			}elseif($this->data['filial'] == '6'){
				return 'Ceara';
			}else{
				return 'Não definido';
			}
